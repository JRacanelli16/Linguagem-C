...
int main(void) {
   ...
   int num = -5, res, tamanho = 45000, contador, vetor1[tamanho], vetor2[tamanho], dif;
   ...
   ftime(&start);
   res = busca_sequencial(vetor1, 0, 28, num);
   if( res == -1){
   ...
   ftime(&start);
   res = busca_binaria(vetor1, 0, 28, num);
   if( res == -1){
      printf("Numero nao encontrado [%d] \n", num);
   }
   else{
...
int busca_binaria(int V[], int inic, int fim, int chave){
   int meio, sup = fim - 1; // limite superior (termina em um número a
